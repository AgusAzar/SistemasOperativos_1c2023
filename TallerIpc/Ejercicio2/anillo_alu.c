...
static void ejecutar_start(int pipe_al_padre[2], int pipe_al_primer_hijo[2],
                           int pipes[][2], int i, int count) {
...
int main(int argc, char **argv) {
  ...
  int pipe_al_padre[2];
  if (pipe(pipe_al_padre) == -1)
    exit(-1);
  if (pipe(pipe_al_primer_hijo) == -1)
    exit(-1);
  for (int i = 0; i < n; ++i) {
    if (pipe(pipes[i]) == -1)
      exit(-1);
