...
{
    int group_index = blockgroup_for_inode(inode_number);
    int inode_index = blockgroup_inode_index(inode_number);
    Ext2FSBlockGroupDescriptor* descriptor = block_group(group_index);
    uint inode_address = descriptor->inode_table + sizeof(Ext2FSInode) * inode_index;
    u_char * buffer_block = (u_char *) malloc(1024 << _superblock->log_block_size);
    u_char * buffer_inodo =(u_char*) malloc(sizeof(Ext2FSInode));
    read_block(inode_address, buffer_block);
    for(int i = 0; i < sizeof(Ext2FSInode); ++i){
        buffer_inodo[i] = buffer_block[i];
    }
    free(buffer_block);
...
{
	unsigned int block_size = 1024 << _superblock->log_block_size;
    unsigned long long addres_in_block = block_size / sizeof(int);
    if(block_number < 12){
        return inode->block[block_number];
    }
    block_number -= 12;
    if(block_number < addres_in_block){
        uint first_page_address = inode->block[12];
        u_char * first_page_buf =(u_char *)malloc(block_size);
        read_block(first_page_address, first_page_buf);
        return ((uint*)first_page_buf)[block_number];
    }
    block_number -= addres_in_block;
    if(block_number < addres_in_block * addres_in_block){
        uint first_page_address = inode->block[13];
        u_char * first_page_buf =(u_char *)malloc(block_size);
        read_block(first_page_address, first_page_buf);
        
        int index_first_page = 0;
        while (block_number > addres_in_block){
            block_number -= addres_in_block;
            ++index_first_page;
        }

        uint second_page_address = ((uint*)first_page_buf)[index_first_page];
        u_char * second_page_buf =(u_char *)malloc(block_size);
        read_block(second_page_address, second_page_buf);

        int index_second_page = block_number;
        return ((uint*)second_page_address)[index_second_page];
    }
}

//uint resolve_indirection(uint block_address, uint block_number, uint level){
//    if(level == 0){
...
//        step_size *= addres_in_block;
//    }
//    uint index_page;
//    while (block_number >= step_size){
//        block_number -= step_size;
//        ++index_page;
//    }
//    u_char * page_buf =(u_char *)malloc(block_size);
//    read_block(block_address, page_buf);
//    uint result = resolve_indirection(((uint*)page_buf)[index_page], block_number, level - 1);
//    free(page_buf);
//    return result;
//}
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	//TODO: Ejercicio 3
	unsigned int block_size = 1024 << _superblock->log_block_size;
    u_char * buffer_block = (u_char *) malloc(block_size);
    int block_number = 0;
    ...
    Ext2FSInode * res;
    while(!encontrado){
        entry = (Ext2FSDirEntry *)buffer_block + entry_offset;
        uint address_block = get_block_address(from,block_number);
        read_block(address_block, buffer_block);
        ++block_number;
        res = entry->inode;
        encontrado = strcmp(filename, entry->name) == 0;
    }
    free(buffer_block);
    return res;
}
